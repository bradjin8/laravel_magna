        ...
        $stations_statistics = DB::select(
            "SELECT ctg.name as category, aa.visit_count as total_visit_count, a.duration_in_sec as total_duration_in_sec, MAX(b.duration_in_sec) as most_duration_in_sec, MAX(c.download_count) as most_download_count FROM
                      ...
                      LEFT JOIN 
                        ...
                        ON ctg.id = a.id
                      LEFT JOIN 
                        (SELECT DISTINCT ct.id as id, COUNT(ct.slug) as visit_count FROM track_records tr JOIN categories ct ON ct.id = tr.category_id AND tr.type='page' AND tr.duration_in_sec=0 GROUP BY tr.category_id) aa
                        ON ctg.id = aa.id
                      LEFT JOIN
